...
///
/// ```xml
///   <foreach> ::= "foreach" <expression> "{" ( <contents> <contents> )* "}"
/// ```

extension TerminalSymbolEnum {

    static let foreachTokens: Set<TerminalSymbolEnum> = TerminalSymbolEnum.blockTokens.union( sortToken )
...
extension ForeachNode {
    ...
    func setSyntax() {
        ...
        for keyword in TerminalSymbolEnum.foreachTokens {
            let entry = AllowableSyntaxEntryStruct( child: keyword, min: 0, max: Int.max )
            allowableChildrenDict[ keyword.description ] = entry
        }
    }

    func isInForeachTokens( _ token: TerminalSymbolEnum ) -> Bool {
        return TerminalSymbolEnum.foreachTokens.contains(token)
    }
    ...
    override func parseSyntaxUsingCompiler( _ compiler: RexselKernel ) throws {
        ...
        while !thisCompiler.isEndOfFile {
            ...
            switch ( thisCompiler.currentToken.type, thisCompiler.nextToken.type, thisCompiler.nextNextToken.type ) {
                ...
                case ( .terminal, _, _ ) where thisCompiler.currentToken.what == .openCurlyBracket && loopExpression.isNotEmpty :
                    isInBlock = true
                    thisCompiler.tokenizedSourceIndex += 1
