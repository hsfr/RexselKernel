...
class GroupingSizeNode: ExprNode  {
    ...
    override init() {
        super.init()
        thisExprNodeType = .groupingSize
        groupingSizeString = "3"
        isLogging = false  // Adjust as required
        setSyntax( options: CountNode.optionTokens, elements: CountNode.blockTokens )
    }
    ...
    override func parseSyntaxUsingCompiler( _ compiler: RexselKernel ) throws {
        ...
        switch ( thisCompiler.currentToken.type, thisCompiler.nextToken.type, thisCompiler.nextNextToken.type ) {
            ...
            case ( .expression, _, _ ) :
                groupingSizeString = thisCompiler.currentToken.value
                thisCompiler.tokenizedSourceIndex += 1
    ...
    ///
    /// ```xml
    ///    <grouping size> ::= “grouuping-size” <quote> <digits> <quote>
    /// ```
   ...
   // -*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
    ...
    override func generate() -> String {
        ...
        return "\(thisExprNodeType.xml)=\"\(groupingSizeString)\""
    }
